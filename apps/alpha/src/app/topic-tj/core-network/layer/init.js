...
const initTooltips = (current, features, projection, bottom = 0, left = 100) => {
  return d3
    .select(current)
    .append('div')
    .attr('class', 'core-svg-tips-group')
    .selectAll('div.core-svg-tips')
    .data(features)
    .enter()
    .append('div')
    .attr('class', 'core-svg-tips')
    .style('left', (d, _) => {
      return left + 'px';
    })
    .style('bottom', (d, _) => {
...
const initCityTooltips = (current, features, projection, offsetX, offsetY) => {
  return d3
    ...
    .style('top', (d, _) => {
      ...
      let position = projection(point || [0, 0]);
      return position[1] + offsetY + 10 + 'px';
    })
    .style('position', 'absolute')
    .attr('name', (d, _) => {
      return d.properties.name;
