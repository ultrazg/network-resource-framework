...
export function ProvincialResources(props:any) {
  ...
  const [successRate, setSuccessRate] = useState<string>('');//达标率
  const [completeRate, setCompleteRate] = useState<string>('');//完成率
  const [exactRate, setExactRate] = useState<string>('');//准确率
  const [totalNum, setTotalNum] = useState<string>('');//总数
  const [successNum, setSuccessNum] = useState<string>('');//成功数
  const getRateData = (data:any) => {
    if(data.map){
      for (let key in data.map) {
        if(key == '5GC' && props.coreType == '5GC核心网'){
          setSuccessRate(data.map[key].successRate);
          setCompleteRate(data.map[key].completeRate);
          setExactRate(data.map[key].exactRate);
          setTotalNum(data.map[key].totalNum);
          setSuccessNum(data.map[key].successNum);
        }
        if(key == 'VIMS' && props.coreType == 'vims'){
          setSuccessRate(data.map[key].successRate);
          setCompleteRate(data.map[key].completeRate);
          setExactRate(data.map[key].exactRate);
          setTotalNum(data.map[key].totalNum);
        ...
        if(key == 'EPC' && props.coreType == '移动核心网'){
          setSuccessRate(data.map[key].successRate);
          setCompleteRate(data.map[key].completeRate);
          setExactRate(data.map[key].exactRate);
          setTotalNum(data.map[key].totalNum);
  ...
  useEffect(() => {
    ...
    else if(props.coreType && qualityData){
            ...
            <div className="top-box">
              ...
              <div className="info-box">
                ...
                <div className="left">
                  ...
                  <div className="num">{ successNum }</div>
                </div>
              </div>
            </div>
              ...
              <div className="info-box">
                  ...
                  <div className="right">
                    <div className="tip-line">
                      ...
                      {successRate && <div className="percent percent3">{successRate}%</div>}
                    </div>
                    <div className="progress-box">
                      <div className="progress progress3">
                        {successRate && <span style={{width: successRate + '%'}}></span>}
                        <div className="dot" style={{left: successRate + '%'}}></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="info-box">
                  ...
                  <div className="right">
                    <div className="tip-line">
                      ...
                      {completeRate && <div className="percent percent2">{completeRate}%</div>}
                    </div>
                    <div className="progress-box">
                      <div className="progress progress2">
                        {completeRate && <span style={{width: completeRate + '%'}}></span>}
                        <div className="dot" style={{left: completeRate + '%'}}></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="info-box">
                  ...
                  <div className="right">
                    <div className="tip-line">
                      ...
                      {exactRate && <div className="percent percent2">{exactRate}%</div>}
                    </div>
                    <div className="progress-box">
                      <div className="progress progress2">
                        {exactRate && <span style={{width: exactRate + '%'}}></span>}
                        <div className="dot" style={{left: exactRate + '%'}}></div>
                      </div>
                    </div>
                  </div>
                </div>
